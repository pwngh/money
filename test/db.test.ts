...
import { DB_VERSION, moneyMysql, moneySql, proveMysql, provePostgres } from '@pwngh/money/db';
import { vectors } from '@pwngh/money';

test('db carrier ships both dialects and a version stamp', () => {
  assert.ok(moneySql.includes('money.div_round') && moneySql.includes('money.split_bps'));
  ...
  assert.ok(moneyMysql.some((s) => s.includes(`select ${DB_VERSION} from dual`)));
});
