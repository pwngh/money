...
writeFileSync(join(out, 'money.vectors.json'), `${JSON.stringify(moneyVectors, null, 2)}\n`);
writeFileSync(join(out, 'fold.vectors.json'), `${JSON.stringify(foldVectors, null, 2)}\n`);
writeFileSync(join(out, 'fold.wasm'), moduleBytes());
writeFileSync(join(out, 'money.sql'), `${moneySql.trim()}\n`);
const mysqlCli = ['delimiter //', ...moneyMysql.map((s) => `${s}//`), 'delimiter ;'].join(
  '\n\n',
);
